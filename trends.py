...
while(i<=len(growingthemes)):
    if(growingthemes[i]>1):
        ...
        grthemes.iloc[d,4]=AnalyticsFrameYTC.iloc[i,1]   
...
if(BRGTflag == 0):
    Ytemptg=Ythemegraph
    Ythemegraph=np.unique(Ythemegraph) 
    ...
    while(i<=im):
        ...
        lm=len(BigStag)
        while(l<=lm):
            if(RepCon.iloc[i,0] == float(BigStag[l]) or RepCon.iloc[i,1] == float(BigStag[l])): 
    ...
    im=len(RepCon.iloc[:,0])
    while(i<=im):
        if(len(BigStag[BigStag == RepCon.iloc[i,0]])>0 and len(BigStag[BigStag == RepCon.iloc[i,1]])>0):
